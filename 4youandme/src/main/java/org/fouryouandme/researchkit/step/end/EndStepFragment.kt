...
class EndStepFragment : StepFragment(R.layout.step_end) {
    ...
    private fun applyData(
        ...
        animationView.addValueCallback(
            ...
            { PorterDuffColorFilter(step.configuration.theme.primaryColorStart.color(), PorterDuff.Mode.SRC_ATOP) }
        )

        animationView.addValueCallback(
            KeyPath( "Line Outlines", "**"),
            LottieProperty.COLOR_FILTER,
            { PorterDuffColorFilter(step.configuration.theme.secondaryColor.color(), PorterDuff.Mode.SRC_ATOP) }
        )
    }
